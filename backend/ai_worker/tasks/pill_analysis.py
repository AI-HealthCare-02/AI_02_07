...
# - 확실하지 않은 각인은 null로 두거나 낙은 confidence로 기록한다.
# - 제조사 로고, 심볼, 비문자 도안은 "마크"로 기록한다.
# - 점, 얼룩, 반점, 점박이 무니, 재질 무니는 각인이 아니다. color_detail 또는 notes에만 기록한다.

# [업로드 패턴 및 회전 대응 — 매우 중요]
# - 첫 번째 이미지=앞면 후보, 두 번째 이미지=뒷면 후보이다.
# - 한 이미지에 동일한 알약의 앞뒷면이 함께 찍힌 경우, 앞면과 뒷면을 각각 구분하여 추출한다.
# - 이 경우 multiple_pills=false이다.
# - 서로 다른 알약이 여러 개 있으면 multiple_pills=true이다.

# [한 이미지에 앞뒷면이 함께 있을 때 각인 회전 처리 — 특히 주의]
# - 동일한 알약의 앞뒷면이 한 이미지에 있으면, 두 면의 각인이 서로 다른 각도로 회전되어 있을 수 있다.
# - 앞면 각인이 0도이고 뒷면 각인이 180도 회전되어 있어도 정상이다.
# - 각 면의 각인을 독립적으로 0/90/180/270도 회전하면서 가장 자연스러운 방향으로 판독한다.
# - 예: 앞면에 MTS가 정상 방향이고 뒷면에 4가 180도 회전되어 7처럼 보이면, 뒷면 각인은 4이다.
# - 예: 앞면에 MTS가 200도 회전되어 있어도 MTS로 판독한다.
# - 회전으로 인한 혼동 주의: 4↔7, 6↔9, 2↔5, d↔0, n↔u, M↔W.
...
# - 예: 1분할선3체럼 보이면 print는 "1 3", left_text="1", right_text="3".
# - 마크가 5, S, JS, 곡선 문자체럼 보여도 문자로 확실하지 않으면 "마크"로 기록한다.

# [OCR 혼동 보정]
# - 다음 혼동은 이미지에서 명확할 때만 보정한다: H↔N, 0↔O, 1↔I↔L, 5↔S, 6↔G, 8↔B, 2↔Z.
# - 회전 혼동: 4↔7, 6↔9, 2↔5, d↔0, n↔u, M↔W.
# - OCR 결과가 한 글자이거나 기호만이면 신뢰하지 않는다.
...
# - 점, 얼룩, 그림자, 하약 가장자리 선은 십자분할선이 아니다.
...
# 알약의 분할선만 다시 확인한다. 각인/색상/모양은 무시한다.
# 반드시 JSON만 출력한다.
...
# _FAINT_IMPRINT_RECHECK_PROMPT = """
# 알약의 희미한 각인만 다시 확인한다. 색상/모양은 무시한다.
# 반드시 JSON만 출력한다.
...
# }"""
_FAINT_IMPRINT_RECHECK_PROMPT = """
Recheck only the pill’s faint imprint. Ignore color and shape.
Output JSON only.
Rules: Speckles, stains, and spotted patterns are not imprints. Only pressed marks, engraved characters, or embossed characters are imprints.
If the letter/number is not certain, use null. If it is a logo or symbol, use "마크".
{
  "print_front": null, "print_back": null,
  "front_candidates": [], "back_candidates": [],
  "imprint_confidence": 0.0,
...
# _MARK_RECHECK_PROMPT = """
# 알약 표면 표시가 문자/숫자인지 마크/로고인지 확인한다.
# 반드시 JSON만 출력한다.
...
# }"""
_MARK_RECHECK_PROMPT = """
Check whether the marking on the pill surface is a letter/number or a mark/logo.
Output JSON only.
Rules: If it looks like a curve, symbol, or company logo and cannot be confirmed as a character, record it as "마크".
A curved mark on a round pill may look like 5, S, or JS. If uncertain, use "마크".
{
  "print_front": null, "print_back": null,
  "front_is_mark": false, "back_is_mark": false,
  "confidence": 0.0,
