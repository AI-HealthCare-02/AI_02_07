...
read -rp "도메인 주소 (예: api.healthguide.com): " DOMAIN
read -rp "이메일 주소 (인증서 갱신 알림): " EMAIL
...
echo ""
echo "🔧 EC2에서 Certbot 실행 중..."

$SSH_CMD << CERTBOT_SCRIPT
    set -e
    ...
    # (Nginx가 80번 포트를 사용 중이면 잠시 중지 필요)
    cd ~/healthguide
    ...
    # 인증서를 Nginx가 읽을 수 있는 위치로 복사
    sudo mkdir -p ~/healthguide/nginx/certs
    sudo cp /etc/letsencrypt/live/$DOMAIN/fullchain.pem ~/healthguide/nginx/certs/
    sudo cp /etc/letsencrypt/live/$DOMAIN/privkey.pem ~/healthguide/nginx/certs/
    sudo chmod 644 ~/healthguide/nginx/certs/*.pem
    ...
    # 자동 갱신 크론 등록
    echo "⏰ 자동 갱신 크론 등록..."
    (sudo crontab -l 2>/dev/null; echo "0 3 * * * certbot renew --quiet --post-hook 'cp /etc/letsencrypt/live/$DOMAIN/fullchain.pem ~/healthguide/nginx/certs/ && cp /etc/letsencrypt/live/$DOMAIN/privkey.pem ~/healthguide/nginx/certs/ && cd ~/healthguide && docker compose restart nginx'") | sudo crontab -

    echo "✅ SSL 인증서 발급 및 자동 갱신 설정 완료!"
    echo "   인증서 위치: /etc/letsencrypt/live/$DOMAIN/"
    echo "   Nginx 인증서: ~/healthguide/nginx/certs/"
    echo ""
    echo "⚠️  nginx/default.conf에서 HTTPS 서버 블록의 주석을 해제하세요."
CERTBOT_SCRIPT

echo ""
echo "========================================"
echo "✅ SSL 인증서 발급 완료!"
echo "   nginx/default.conf의 HTTPS 블록 주석을 해제하고"
echo "   server_name을 $DOMAIN으로 변경한 후"
echo "   docker compose restart nginx 를 실행하세요."
echo "========================================"