...
-- 카카오 토큰 컨럼 추가 (카카오 메시지 API 사용 용)
DO $$ BEGIN
    ALTER TABLE users ADD COLUMN IF NOT EXISTS kakao_access_token VARCHAR(512);
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

DO $$ BEGIN
    ALTER TABLE users ADD COLUMN IF NOT EXISTS kakao_refresh_token VARCHAR(512);
EXCEPTION WHEN OTHERS THEN NULL;
END $$;
