...
class TestLoginAPI:
    ...
    async def test_dev_login_with_email(self, client, test_user):
        """특정 이메일로 개발용 로그인 성공."""
        response = await client.post(f"/api/v1/auth/dev/login?email={test_user.email}")
        assert response.status_code == status.HTTP_200_OK
        assert response.json()["data"]["access_token"]

    async def test_dev_login_nonexistent_email(self, client):
        """존재하지 않는 이메일로 개발용 로그인 시 404."""
        response = await client.post("/api/v1/auth/dev/login?email=nonexistent@example.com")
