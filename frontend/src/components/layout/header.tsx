...
function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggleTheme}
      className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-muted text-muted-foreground transition hover:text-foreground"
      aria-label="테마 전환"
    >
      {theme === "dark" ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden lg:block">
              {isAuthenticated ? (
                ...
                <Link
                  href="/login"
                  className="rounded-full border border-teal-500/30 bg-teal-500/5 px-5 py-2 text-sm font-medium text-teal-600 dark:text-teal-300 backdrop-blur-sm transition-all hover:border-teal-400/60 hover:bg-teal-500/15"
                >
                  로그인
                </Link>
              )}
